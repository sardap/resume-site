...
function getNextPhrase() {
    ...
    if (englishAnswers) {
        currentQuestion.value.question = currentPhrase.korean[Math.floor(Math.random() * currentPhrase.korean.length)].text;
        currentQuestion.value.options = [currentPhrase.english[Math.floor(Math.random() * currentPhrase.english.length)].text];
    } else {
        currentQuestion.value.question = currentPhrase.english[Math.floor(Math.random() * currentPhrase.english.length)].text;
        currentQuestion.value.options = [currentPhrase.korean[Math.floor(Math.random() * currentPhrase.korean.length)].text];
    }
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (englishAnswers) {
            nextOption = randomPhrase.english[Math.floor(Math.random() * randomPhrase.english.length)].text;
        } else {
                    ...
                    <div class="table-container">
                        <table>
                            <tr v-for="option, i in currentQuestion.options">
                                <td><input :id="`quiz_time_radio_${option}`" :name="`quiz_time_radio_${option}`"
                                        type="radio" :value="option" v-model="answer" />
                                </td>
                                <td><label :id="`quiz_time_radio_${option}`" :for="`quiz_time_radio_${option}`">{{ option
                                }}</label>
                                </td>
                                <td>
